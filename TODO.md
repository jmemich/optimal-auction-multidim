...
- [ ] **Direct analytic test of the *approximation* (N=1)** — the Pavlov/Myerson closed-form tests validate the *EBM oracle*, not the LP. We only test the approximation via (a) the baseline regression (its own historical outputs — partly circular) and (b) approx→EBM convergence. A direct check is missing. Note: the LP allows randomisation (Q∈[0,1]), so at N=1 on U[0,1]² it should recover the *randomised* Pavlov optimum (~0.3895, ~1.2% above the deterministic 0.3849 — see benchmarks.md), not 0.3849. That gap is itself the "approximation detects stochasticity" result worth asserting.
