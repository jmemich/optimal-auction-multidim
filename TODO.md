...
- [x] **Direct analytic tests of the *approximation* (N=1)** — `tests/validation/test_approximation_analytic.py`: the LP (not the oracle) Richardson-extrapolates to closed forms. **Pavlov** (U[0,1]²) → 2/(3√3)≈0.38490; **Myerson** (single good via a degenerate, unprofitable good 2) → 0.25. Non-circular — analytic targets, not the LP's own stored output. **Correction:** randomisation does *not* help at U[0,1]²; the LP converges to the *deterministic* optimum here (it would beat it for shifted supports such as U[2,3]², setting 1b).
